...
const Validazione: React.FC = () => {
  ...
  const handleSearch = async (searchId: string) => { // Aggiunto un parametro per l'ID
    setLoading(true);
    setError('');
    try {
  ...
  return (
    <section className='flex flex-col items-center justify-center bg-white text-black h-[100vh] w-full gap-6'>
            ...
            <button onClick={() => handleSearch(id)} disabled={loading}
              className='bg-black text-white text-lg flex w-80 p-2 gap-2 items-center justify-center uppercase rounded-full'>
